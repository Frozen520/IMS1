				...
				<form action="" id="filter">
					<div class="row align-items-end">
						...
						<div class="form-group col-md-4">
							...
							<a href="mailto:?subject=Monthly Report&body=Please find the attached report." class="btn btn-sm btn-flat btn-info" id="email_report"><i class="fa fa-envelope"></i> Email Report</a>
						</div>
					</div>
				</form>
			</fieldset>
			...
			<div class="container-fluid">
				<div class="row">
					...
					<div class="col-8">
						...
						<h3 class="text-center"><b>Monthly Insurance Transaction Report</b></h3>
				...
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
                        ...
                        SELECT 
                            i.*,
                            CONCAT(p.code,' - ', p.name) as `policy`, 
                            p.duration, 
                            p.doc_path, 
                            c.name as category,
                            ord.Status as stt,
                            ord.order_id as orderID,
                            CONCAT(cc.code,' - ',cc.lastname, ', ', cc.firstname, ' ', cc.middlename) as client 
                        FROM `insurance_list` i 
                        inner join  policy_list p on i.policy_id = p.id 
                        inner join category_list c on p.category_id = c.id 
                        inner join client_list cc on i.client_id = cc.id 
                        inner join orders ord on ord.productId = i.id
                        where date(i.registration_date) between '{$from}' and '{$to}' 
                        order by date(i.registration_date) asc 
                        ");
						while ($row = $qry->fetch_assoc()) :
							$total += $row['cost'];
							$payNow = ($row['stt'] != 1) ? '<a href="../pay.php?amount=' . format_num($row['cost']) . '&phone=0722&orderID=' . $row['orderID'] . '">Pay Now</a>' : "";
							$stt = ($row['stt'] == 1) ? 'Paid' : 'Unpaid';
						?>
							<tr>
								...
								<td class="">
									<p class="m-0"><?php echo $row['client'] ?></p>
								</td>
								<td class="">
									<p class="m-0"><?php echo $row['policy'] ?></p>
								</td>
								<td class="">
									<p class="m-0"><?php echo $row['registration_no'] ?></p>
								</td>
